                        ...
                        try:
                            ...
                            if pad_list:
                                ...
                                for pad in pad_list:
                                    ...
                                    # Estimate pad extent (use 1mm default if size unavailable)
                                    pw = to_mm(pad.size.x) / 2 if hasattr(pad, 'size') and pad.size else 0.5
                                    ph = to_mm(pad.size.y) / 2 if hasattr(pad, 'size') and pad.size else 0.5
                                    min_x = min(min_x, px - pw)
                                    min_y = min(min_y, py - ph)
                                    max_x = max(max_x, px + pw)
                                    max_y = max(max_y, py + ph)
                                # Add small margin for component body beyond pads
                                margin = 0.25  # mm
                                bbox_data = {
                                    "min_x": min_x - margin,
                                    "min_y": min_y - margin,
